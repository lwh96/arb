...
# --- Configuration ---
# Taker fees in decimal (e.g. 0.046% = 0.00046)
EXCHANGE_TAKER_FEES = {
    "binance": 0.00046,
    "bybit": 0.00055,
    "bitget": 0.00060,
...
MIN_VOLUME_USD = 500_000        # Absolute floor for volume
IDEAL_VOLUME_USD = 10_000_000   # Volume where liquidity multiplier hits ~1.0
...
class FundingArbitrageScorer:
    ...
    def score_opportunities(self, market_data: List[ArbitrageMarketData]) -> List[Opportunity]:
        ...
        pairs = pairs[pairs['gross_yield_bps'] > 0].copy() # Filter direction
        if pairs.empty: return []

        def get_fee(ex_series):
            return ex_series.map(EXCHANGE_MAKER_FEES).fillna(EXCHANGE_MAKER_FEES['default'])
        
        pairs['fees_bps'] = (get_fee(pairs['exchange_L']) + get_fee(pairs['exchange_S'])) * 2 * 10_000
        ...
        # 6. Scoring
        spread_cost_bps = np.minimum(pairs['entry_spread_bps'], 0)
        pairs['est_net_profit_bps'] = pairs['gross_yield_bps'] - pairs['fees_bps'] + spread_cost_bps
        
        pairs = pairs[pairs['est_net_profit_bps'] > 0].copy()
        if pairs.empty: return []
        ...
        spread_bonus = np.maximum(pairs['entry_spread_bps'], 0) * 0.2
        basis_penalty = pairs['mark_divergence_bps'] * 0.5
        
        raw_score = (pairs['est_net_profit_bps'] + spread_bonus - basis_penalty) * pairs['liquidity_score']
        ...
        for _, row in pairs.iterrows():
            opp = Opportunity(
                ...
                time_to_funding_min=round((float(row['earliest_ts']) - current_time) / 60_000, 1),
